...
class TestVanguardPositions(unittest.TestCase):
    def setUp(self) -> None:
        self.positions = list(
            vanguard.VanguardAccount(
                statement=Path("tests/vanguard_positions_and_transactions.csv")
            ).positions()
        )
    ...
    def test_vmfxx(self) -> None:
        self.assertEqual(self.positions[1].instrument, Stock("VMFXX", Currency.USD))
        self.assertEqual(self.positions[1].quantity, Decimal("543.21"))
        # TODO: Validate cost basis

    def test_voo(self) -> None:
        self.assertEqual(self.positions[2].instrument, Stock("VOO", Currency.USD))
        self.assertEqual(self.positions[2].quantity, Decimal("100.1"))
        # TODO: Validate cost basis

    def test_vt(self) -> None:
        self.assertEqual(self.positions[3].instrument, Stock("VT", Currency.USD))
        self.assertEqual(self.positions[3].quantity, Decimal("10"))
        # TODO: Validate cost basis

    def test_vti(self) -> None:
        self.assertEqual(self.positions[4].instrument, Stock("VTI", Currency.USD))
        self.assertEqual(self.positions[4].quantity, Decimal("50.5"))
        # TODO: Validate cost basis

    def test_vbr(self) -> None:
        self.assertEqual(self.positions[5].instrument, Stock("VWO", Currency.USD))
    ...
    def test_buySecurity(self) -> None:
        ...
        self.assertEqual(
            ...
            Trade(
                ...
                quantity=Decimal("12"),
                amount=Cash(currency=Currency.USD, quantity=Decimal("-3456.78")),
    ...
    def test_sellSecurity(self) -> None:
        ...
        self.assertEqual(
            ...
            Trade(
                ...
                quantity=Decimal("-4"),
                amount=Cash(currency=Currency.USD, quantity=Decimal("1234.56")),
                fees=Cash(currency=Currency.USD, quantity=Decimal("0.00")),
    ...
    def test_redeemTBill(self) -> None:
        ...
        self.assertEqual(
            ...
            Trade(
                ...
                amount=Cash(currency=Currency.USD, quantity=Decimal("9987.65")),
                fees=Cash(currency=Currency.USD, quantity=Decimal("0.00")),
                flags=TradeFlags.CLOSE,
            ),
        )
...
class TestVanguardBalance(unittest.TestCase):
    def setUp(self) -> None:
        self.balance = vanguard.VanguardAccount(
            statement=Path("tests/vanguard_positions_and_transactions.csv")
        ).balance()
